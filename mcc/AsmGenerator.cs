    ...
    class AsmGenerator
    {
        ...
        private void GenerateDeclaration(ASTDeclarationNode dec)
        {
            if (dec.IsGlobal)
            {
                ...
                {
                    backend.GenerateGlobalVariable(dec.Name, Evaluate(dec.Initializer));
                }
            }
            else
            {
        ...
        private static int Evaluate(ASTAbstractExpressionNode exp)
        {
            switch(exp)
            {
        ...
        private static int EvaluateConstant(ASTConstantNode constant)
        {
            return constant.Value;
        }

        private static int EvaluateUnaryOp(ASTUnaryOpNode unaryOp)
        {
